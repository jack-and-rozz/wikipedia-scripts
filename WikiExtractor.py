...
class Extractor(object):
    ...
    def write_output(self, out, text):
        ...
        if options.write_json:
            ...
            footer = "\n</doc>\n"
            #footer = "\n</doc>\n\n" # empty line is needed after the end of a doc.
            if out == sys.stdout:   # option -a or -o -
                header = header
            out.write(header)
            for line in text:
    ...
    def wiki2text(self, text):
        ...
        if options.toHTML:
            ...
            # MODIFIED. the first bolded phrase in an article is regarded as a link to itself.
            text = bold.sub(r'[[%s|\1]]' % self.title, text, count=1)
...
parserFunctions = {
    ...
    'urlencode': lambda extr, string, *rest: quote(string),
...
def pages_from(input):
    ...
    title = None
    for line in input:
        if not isinstance(line, text_type): line = line
