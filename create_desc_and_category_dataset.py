...
@common.timewatch
def merge(data, contexts, n_max_context):
  ...
  for qid in data:
    ...
    if contexts[qid]:
      ...
      new_data[qid].categories = categories
...
def statistics(data):
  ...
  category_cnt = collections.Counter(common.flatten([d.categories for d in data.values()]))
...
def main(args):
  ...
  data = merge(data, contexts_by_qid, args.n_max_context)
  ...
  # Plot frequencies of category labels.
  category_freq = collections.Counter(common.flatten([d.categories for d in data.values()]))

  with open(target_dir + '/category_freq.log', 'w') as f:
    sys.stdout = f
    for k, v in sorted(category_freq.items(), key=lambda x:-x[1]):
      print(k, v)
    sys.stdout = sys.__stdout__
    
  title = 'Category frequency histgram'
  histgram(category_freq.values(),
           target_dir + '/category_histgram.png', title)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--n_dev', type=int, default=3000)
